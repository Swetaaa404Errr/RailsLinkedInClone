...
class JobFieldsController < ApplicationController
  ...
  def create
    ...
    if @job_field.save
      redirect_to job_fields_path, notice: 'Successfully added a jobsector'
    else
  ...
  def update
    ...
    if @job_field.update(job_field_params)
      ...
      redirect_to job_fields_path, notice: 'Successfully updated jobfield'
    else
  ...
  def destroy
    ...
    if @job_field.destroy
      redirect_to job_fields_path, notice: 'Successfully deleted a jobield'
    else
      redirect_to job_fields_path, notice: 'Couldnt delete a jobfield'
    end
  end
